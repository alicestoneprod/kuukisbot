        ...
        .then((response) => {
          ...
          weather.getSmartJSON(function (err, json) {
            if (err) {
              ...
              const pressure = Math.round(json.pressure / 1.333)
              let resultCity = cityName.split("")
              resultCity[0] = resultCity[0].toUpperCase()
              resultCity = resultCity.join("")
              const formattedMessage = `⛅️ По запросу погоды в г.${resultCity} на данный момент известно следующее:
🌡️ Температура воздуха: ${Math.round(json.temp) + 2}°C, ${json.description}
💧 Процент влажности: ${json.humidity}%.
